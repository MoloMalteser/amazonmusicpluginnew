...
export default definePlugin({
  title: "Amazon Music Plugin",   // Titel für Decky
  icon: "🎵",                     // Icon für Decky Store
  content: (serverAPI?: ServerAPI) => {
    ...
    const fetchRecommendations = async () => {
      ...
      if ('result' in recs) setRecommendations(recs.result as any[]);
    };

    const fetchRecentlyPlayed = async () => {
      const recentSongs = await serverAPI!.callPluginMethod("recently_played", {});
      if ('result' in recentSongs) setRecent(recentSongs.result as any[]);
    };

    const searchSongs = async () => {
      if (!query) return;
      const results = await serverAPI!.callPluginMethod("search_songs", { query });
      if ('result' in results) setSearchResults(results.result as any[]);
    };
